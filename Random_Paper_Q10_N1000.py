...
for e in range(Experiments):
	...
	model.add(Convolution2D(nb_filters*2, nb_conv, nb_conv, border_mode='valid', input_shape=input_shape))
	model.add(Activation('relu'))
	...
	hist = model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=nb_epoch, verbose=1, validation_data=(X_valid, Y_valid))
	...
	for i in range(acquisition_iterations):
		...
		model = Sequential()
		model.add(Convolution2D(nb_filters, kernel_size=(nb_conv, nb_conv), padding ='same', input_shape=input_shape))
		model.add(Activation('relu'))
		model.add(Convolution2D(nb_filters, kernel_size=(nb_conv, nb_conv)))
		model.add(Activation('relu'))
		model.add(MaxPooling2D(pool_size=(nb_pool, nb_pool)))
		model.add(Dropout(0.25))

		model.add(Convolution2D(nb_filters*2, kernel_size=(nb_conv, nb_conv),padding ='same', input_shape=input_shape))
		model.add(Activation('relu'))
