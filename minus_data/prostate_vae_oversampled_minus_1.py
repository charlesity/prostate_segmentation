...
def visualize(X_in, y_in):
    ...
    ax.scatter(X_in[y_in == 0, 0], X_in[y_in == 0, 1], X_in[y_in == 0, 2],  label="Class #0", alpha=0.5)
    ax.scatter(X_in[y_in == 1, 0], X_in[y_in == 1, 1], X_in[y_in == 1, 2],  label="Class #1", alpha=0.5)
    ax.set_title("Over Samplying SMOTE and Slice Number Feature "+ str(slice_number_feature))
...
print ( (y_test[y_test==0]).shape, "Negative y After Split for VAE Over Sampling" )
print ( (y_test[y_test==1]).shape, "Positve y After Split for VAE Over Sampling" )
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if slice_number_feature:
            vae.save_weights('prostate_vae_extra_feature_with_minus_1.h5')
        else:
            vae.save_weights('prostate_vae_without_extra_feature_with_minus_1.h5')          
    ...
    # X_res = kpca.fit_transform(x_test)
    # print (X_res.shape)
    # fig = plt.figure()
    ...
    # # ax.scatter(X[y == 0, 0], X[y == 0, 1],X[y == 0, 2],  label="Class #0", alpha=0.5)
    # ax.scatter(X_res[y_test == 0, 0], X_res[y_test == 0, 1], X_res[y_test == 0, 2],  label="Class #0", alpha=0.5)
    # ax.scatter(X_res[y_test == 1, 0], X_res[y_test == 1, 1], X_res[y_test == 1, 2],  label="Class #1", alpha=0.5)
    # ax.set_xlabel('First Component')
    # ax.set_ylabel('Second Component')
    # ax.set_zlabel('Third Component')
    # plt.legend()
