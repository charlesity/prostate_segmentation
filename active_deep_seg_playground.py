...
dropout_iterations = 50  # number of dropout ROUNDS for uncertainty estimation
...
for train_index, test_index in sss:
    ...
    datagen = ImageDataGenerator(zca_whitening=True, featurewise_center = True, featurewise_std_normalization=True, validation_split = .33)
    datagen.fit(X_Train)
    ...
    model.fit_generator(datagen, X_Train, Y_Train, batch_size = batch_size, nb_epochs= nb_epoch)
...
#         model = net(X_Train, Y_Train,input_shape= input_shape, filters= nb_filters, kernel_size=k_size, batch_size=batch_size, maxpool=mp_size, 
#         droput_iteration=dropout_iterations, n_epochs = nb_epoch, normalize = False, dropout = dropout)
...
#         average_precision = metrics.average_precision_score(
#             y_reverted, y_predicted[:, 1])
#         print('Average Precision ', average_precision, ' acquisition number ', i+1, 'experiment number ', e+1)
